...
| 2026-06-25 | Strip all dev-branch references from the README (Guard.Tests, claude-dev links/branch, Pester, requirement IDs, Companion docs section) | Dev planning lives in the planning docs; README pointers to dev artifacts aren't needed on either branch and broke on `main` (tests/ excluded). Side effect: README now identical on both branches → release no longer needs the companion-link trim. |
| 2026-06-25 | Repo made **PUBLIC** | Maintainer decision. Triggers the RELEASE_TO_MAIN re-evaluation: force-push/orphan-rebuild rewrites the public default branch's history (OPEN — pick a non-force model or drop the two-branch split). `claude-dev` (planning, `.ai-reviews/`, `background/`) is now world-readable; goal.md is generic (no client data), keys + real configs remain gitignored. |
